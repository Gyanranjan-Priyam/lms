...
export function CourseStructure({ data }: iAppProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if(!targetChapterId) {
        toast.error("Unable to reorder chapter");
        return;
    }
    ...
    const reoderedLocalChapters = arrayMove(items, oldIndex, newIndex);
    const updatedChapterForState = reoderedLocalChapters.map((chapter, index) => ({
        ...chapter,
        order: index + 1,
    }));
    ...
    if(activeType === "lesson" && overType === "lesson") {
        ...
        const chapterIndex = items.findIndex((chapter) => chapter.id === chapterId);
            if(chapterIndex === -1) {
                toast.error("Could not find chapter to toggle");
                return;
            }
    ...
    <DndContext collisionDetection={rectIntersection} onDragEnd={handleDragEnd} sensors={sensors}>
      <Card>
        ...
        <CardContent className="space-y-8">
          ...
          >
            {items.map((item) => (
                <SortableItem id={item.id} key={item.id} data={{ type: 'chapter' }}>
                    {(listeners) => (
                        <Card>
                            <Collapsible 
                                open= {item.isOpen}
                                                        ...
                                                        {(lessonListeners) => (
                                                            <div className="flex items-center justify-center p-2 hover:bg-accent rounded-sm">
                                                                <div className="flex items-center gap-2 flex-1">
                                                                    <Button variant="ghost" size="icon" {...lessonListeners} className="cursor-grab opacity-60 hover:opacity-100">
                                                                        <GripVertical className="size-4" />
                                                                    </Button>
