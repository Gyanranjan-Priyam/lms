...
import { HomeIcon, BookOpen, LayoutDashboard } from "lucide-react";
        ...
        <header className="sticky top-0 z-50 w-full border-b bg-background/95
            ...
            <div className="container flex min-h-16 items-center mx-auto px-4 md:px-6 lg:px-8">
                <Link href="/" className="flex items-center space-x-2 mr-4">
                    ...
                    <span className="font-bold">Sams School</span>
                </Link>

        {/* desktop navigation */}
                <nav className="hidden md:flex md:flex-1 md:items-center md:justify-between ">
                    <div className="flex items-center mx-auto justify-between space-x-14">
                        {navigationItems.map((item) =>(
                            <Link key={item.name} href={item.href} className="text-sm font-medium transition-colors hover:text-primary">
                                <item.icon className="inline-block mr-2 mb-1 size-4" />{item.name}
                            </Link>
                        ) )}
                    </div>
